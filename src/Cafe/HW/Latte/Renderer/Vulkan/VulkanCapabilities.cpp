...
bool VulkanCapabilities::s_supportsQcomRenderPassTransform = false;
bool VulkanCapabilities::s_supportsQcomImageProcessing = false;
bool VulkanCapabilities::s_supportsQcomTileProperties = false;
bool VulkanCapabilities::s_supportsQcomFragmentDensityMapOffset = false;
...
void VulkanCapabilities::Initialize(VkPhysicalDevice device) {
	...
	if (extCount > 0) {
		...
		for (const auto& ext : extensions) {
			// Extensions Qualcomm de base
			if (strcmp(ext.extensionName, "VK_QCOM_render_pass_transform") == 0) {
				s_supportsQcom = true;
				s_supportsQcomRenderPassTransform = true; // NOUVEAU
			}

			// NOUVEAU - Autres extensions Qualcomm pour optimisations
			if (strcmp(ext.extensionName, "VK_QCOM_image_processing") == 0) {
				s_supportsQcomImageProcessing = true;
			}

			if (strcmp(ext.extensionName, "VK_QCOM_tile_properties") == 0) {
				s_supportsQcomTileProperties = true;
			}

			if (strcmp(ext.extensionName, "VK_QCOM_fragment_density_map_offset") == 0) {
				s_supportsQcomFragmentDensityMapOffset = true;
			}
...
// 4. Log des capabilities détectées (peut être enlevé en production)
	...
	// NOUVEAU - Log détaillé des extensions Qualcomm
	if (s_isAdreno) {
		printf("VulkanCapabilities: QCOM Render Pass Transform: %s\n",
			   s_supportsQcomRenderPassTransform ? "Yes" : "No");
		printf("VulkanCapabilities: QCOM Image Processing: %s\n",
			   s_supportsQcomImageProcessing ? "Yes" : "No");
		printf("VulkanCapabilities: QCOM Tile Properties: %s\n",
			   s_supportsQcomTileProperties ? "Yes" : "No");
		printf("VulkanCapabilities: QCOM Fragment Density Map Offset: %s\n",
			   s_supportsQcomFragmentDensityMapOffset ? "Yes" : "No");
	}
